...
export function getTimeSlotInfo(
  ...
  const slotCourse = timetable.timetable.filter((course) => {
    return course.slots.some((course_slot) =>
      course_slot.every((value, index) => value === searchSlot[index])
    );
  });

  if (slotCourse.length > 0) {
    return {
      occupied: true,
      // course_code: slotCourse.course_code,
      // course_name: slotCourse.course_name,
      // rooms: slotCourse.rooms
      courses: slotCourse.map((course) => {
        return {
